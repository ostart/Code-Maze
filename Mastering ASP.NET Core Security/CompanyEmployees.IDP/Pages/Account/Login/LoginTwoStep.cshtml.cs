...
public class LoginTwoStepModel : BasePage
{
    ...
    public async Task<IActionResult> OnGet(string email, bool rememberLogin, string returnUrl)
    {
        ...
        var message = new Message([email], "Authentication token", token, null);
    ...
    public async Task<IActionResult> OnPost()
    {
        ...
        var result = await _signInManager.TwoFactorSignInAsync("Email", Input.TwoFactorCode, 
            Input.RememberLogin, rememberClient: false);
        if (result.Succeeded)
        {
